...
The above procedure uses $$\theta{(n)}$$ steps and $$\theta{(n)}$$ space. Here is an iterative procedure that uses $$\theta{(1)}$$ space:
...
The above procedure grows logarithmically with n ($$\theta{(log(n))}$$) in both space and time.

The following procedure generates an iterative process which uses constant space ($$\theta{(1)}$$) along with the technique for fast exponentiation described above:
